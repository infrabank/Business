  ...
  return (
    <div className={compact ? 'space-y-2' : 'space-y-4'}>
      <div className={compact ? 'space-y-1.5' : 'grid grid-cols-1 gap-3 md:grid-cols-2'}>
        {variables.map((v) => (
          <div key={v.name}>
            <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-500">
              {`{{${v.name}}}`}
            </label>
            <input
              type="text"
      ...
      {!compact && (
        <div>
          <p className="mb-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-500">
            최종 프롬프트 미리보기
          </p>
          <pre className="max-h-40 overflow-y-auto whitespace-pre-wrap rounded-xl bg-slate-50 dark:bg-slate-800/50 p-3 text-sm text-slate-700 dark:text-slate-300 dark:text-slate-500">
            {systemPrompt
