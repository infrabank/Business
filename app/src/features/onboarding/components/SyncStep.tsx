  ...
  if (syncStatus === 'idle' || syncStatus === 'syncing') {
    return (
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">잠시만 기다려 주세요...</p>
      ...
      {hasData ? (
        <div className="mx-auto mt-6 grid max-w-xs grid-cols-2 gap-4">
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">총 비용</p>
          </div>
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{syncSummary!.totalRequests.toLocaleString()}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">요청 수</p>
          </div>
        </div>
      ) : (
        <div className="mx-auto mt-6 max-w-sm rounded-xl border border-blue-100 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/50 p-4">
