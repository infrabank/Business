  ...
  return (
    <div className="rounded-2xl border border-slate-200/60 bg-white shadow-2xl overflow-hidden">
      ...
      <div className="p-4 lg:p-6">
        ...
        <div className="grid grid-cols-3 gap-3 rounded-lg border border-slate-100 bg-white p-4 shadow-sm">
          <div className="rounded-lg bg-slate-50 p-4 text-center">
            <p className="text-[10px] font-medium text-slate-400 uppercase">LCM 없이</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">$8,247</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-lg border border-slate-100 bg-white p-3 shadow-sm">
            <p className="mb-2 text-xs font-semibold text-slate-700">절감 내역</p>
            <div className="space-y-2">
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-slate-600">응답 캐싱</span>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-slate-600">스마트 모델 라우팅</span>
          ...
          <div className="rounded-lg border border-slate-100 bg-white p-3 shadow-sm">
            <p className="mb-2 text-xs font-semibold text-slate-700">최근 최적화</p>
        ...
        <div className="mt-4 rounded-lg border border-slate-100 bg-white p-3 shadow-sm">
          <p className="mb-2 text-xs font-semibold text-slate-700">실시간 요청 로그</p>
          <div className="space-y-1">
            {[
              ...
              <div key={i} className="flex items-center gap-2 rounded bg-slate-50 px-2 py-1">
                <code className="w-24 text-[10px] text-slate-600 truncate">{row.model}</code>
                {row.from && <span className="text-[9px] text-slate-400">from {row.from}</span>}
                <span className={`rounded px-1 py-0.5 text-[9px] font-medium ${
                  row.intent === 'coding' || row.intent === 'reasoning' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-500'
                }`}>{row.intent}</span>
                ...
                <span className="text-[10px] font-semibold text-slate-900">{row.cost}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="bg-gradient-hero">
      <section className="mx-auto max-w-6xl px-4 py-24 lg:py-32 text-center">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-indigo-50 border border-indigo-200/50 px-5 py-2 text-sm font-semibold text-indigo-700">
          <Zap className="h-4 w-4" /> 절약해드릴 때까지 무료 — 그 후 절감액의 20%만
        </div>

        <h1 className="mx-auto max-w-4xl text-5xl lg:text-7xl font-bold leading-[1.1] tracking-tight text-slate-900">
          LLM 비용을{' '}
        ...
        <p className="mx-auto mt-6 max-w-2xl text-lg lg:text-xl text-slate-600 leading-relaxed">
          ...
          <span className="font-semibold text-slate-900">동일한 결과, 비용은 절반 이하.</span>
        </p>
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          ...
          <Link
            href="/pricing"
            className="inline-flex items-center gap-2 rounded-xl border-2 border-slate-200 px-8 py-4 text-base font-semibold text-slate-700 transition-all duration-200 hover:border-slate-300 hover:bg-slate-50"
          >
            요금제 보기
          </Link>
        </div>
