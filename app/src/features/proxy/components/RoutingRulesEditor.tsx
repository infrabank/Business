...
export function RoutingRulesEditor({ routingMode, rules, onChange }: Props) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 p-4 space-y-4">
      <div>
        <p className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">라우팅 모드</p>
          ...
          {(['auto', 'manual', 'off'] as const).map((mode) => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${
                routingMode === mode ? 'bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 shadow-sm' : 'text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100'
              }`}
            >
          ...
          <div className="flex flex-wrap items-end gap-2">
            <div className="space-y-1">
              ...
              <select
                ...
                className="block w-36 rounded border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 px-2 py-1 text-xs"
              >
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600 dark:text-slate-400">조건</label>
              <select
                value={newCondition}
                onChange={(e) => setNewCondition(e.target.value as RoutingRule['condition'])}
                className="block w-32 rounded border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 px-2 py-1 text-xs"
              >
